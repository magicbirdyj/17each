...
<script  type="text/javascript">
    ...
    <div class="release">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="content"><!--内容遍历-->
            ...
            <div class='td3'><a href="<?php echo U('Goods/index',array('goods_id'=>$vo['goods_id']));?>"><img src="<?php echo ($vo['goods_img']); ?>" class="img_zst"/></a></div>
            <div class='td4'><?php echo ($vo['price']); ?></div>
            <div class='td5'><?php echo (date('Y/m/d H：i',$vo['add_time'])); ?></div>
            <div class='td6'><a href="<?php echo U('Member/goods_editor',array('goods_id'=>$vo['goods_id']));?>">编辑</a><a href="javascript:void(0);" onclick="Del(<?php echo ($vo['goods_id']); ?>,'<?php echo ($vo['goods_name']); ?>')">下架</a></div>
...
<script type="text/javascript">
    function Del(id,name){
        if(window.confirm('确定要下架商品 ['+name+'] 吗？')){
            var url="/Home/Member/goods_del/goods_id/"+id+".html";
            $.ajax({
                        type:'get',
                        async : false,
                        url:url,
