...
<script  type="text/javascript">
    ...
    <div class="release">
        <select class="order_select">
            <option value="<?php echo U('Order/index');?>" <?php echo ($selected['all']); ?> >全部状态</option>
            <option value="<?php echo U('Order/index?status=no_pay');?>" <?php echo ($selected['no_pay']); ?> >未付款</option>
            <option value="<?php echo U('Order/index?status=daiqueren');?>" <?php echo ($selected['daiqueren']); ?> >待确认</option>
            <option value="<?php echo U('Order/index?status=daipingjia');?>" <?php echo ($selected['daipingjia']); ?> >待评价</option>
        </select>
        <div class="kuaijietishi">快捷提示：您有
            <a href="<?php echo U('Order/index');?>" class="em">全部状态（<?php echo ($status_count['all']); ?>）</a>
            <a href="<?php echo U('Order/index?status=no_pay');?>" class="em">待付款（<?php echo ($status_count['no_pay']); ?>）</a>
            <a href="<?php echo U('Order/index?status=daiqueren');?>" class="em">待确认（<?php echo ($status_count['daiqueren']); ?>）</a>
            <a href="<?php echo U('Order/index?status=daipingjia');?>" class="em">待评价（<?php echo ($status_count['daipingjia']); ?>）</a>
        </div>
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="content"><!--内容遍历-->
            ...
            <div class="order_row">
                <div class='td1'>
                    ...
                    <div class="info"><a href="<?php echo U('Goods/index',array('goods_id'=>$vo['goods_id']));?>"><?php echo ($vo['goods_name']); ?></a><p class="color_999">商品日期：<?php echo (date_geshi1($vo['server_day'])); ?></p><p class="color_2BB8AA">商家：<?php echo ($vo['shop_name']); ?></p></div>
                </div>
                <div class='td2'>&yen;<?php echo ($vo['price']); ?></div>
                <div class='td3'>&yen;<?php echo ($vo['dues']); ?></div>
                <div class='td4'><p><?php echo order_status($vo['pay_status'],$vo['status'],$vo['order_id'],$vo['server_day'],$vo['goods_id'])['status'];?></p><a href="<?php echo U('Order/view',array('order_id'=>$vo['order_id']));?>">订单详情</a></div>
                <div class='td5'>
                    <?php if($vo['pay_status'] == '3' ): ?><a href="<?php echo ($vo['order_id']); ?>" class="button_a_lvse" style="margin-bottom: 20px;">删除订单</a><?php endif; ?>
                    <a href="<?php echo order_status($vo['pay_status'],$vo['status'],$vo['order_id'],$vo['server_day'],$vo['goods_id'])['status_url'];?>" class="button_a_lvse"><?php echo order_status($vo['pay_status'],$vo['status'],$vo['order_id'],$vo['server_day'],$vo['goods_id'])['status_button'];?></a>
                    <?php if(($vo['pay_status'] == '0') and ($vo['status'] != '4')): ?><a id='quxiao_order' class="button_a_lvse" name="<?php echo ($vo['order_id']); ?>" style="margin-top: 20px;">取消订单</a><?php endif; ?>
                    <?php if(($vo['pay_status'] == '1') and ($vo['status'] == '1') and (shijianchuo($vo['server_day']) > $time)): ?><a id='tuikuang' class="button_a_lvse" name="<?php echo ($vo['order_id']); ?>" href='<?php echo U("Order/tuikuang",array("order_id"=>$vo["order_id"]));?>' style="margin-top: 20px;">申请退款</a><?php endif; ?>
                    
                    <?php if((($vo['pay_status'] == '1') and ($vo['status'] != '1')) or (shijianchuo($vo['server_day']) < $time) ): ?><a id='shouhou' class="button_a_lvse" name="<?php echo ($vo['order_id']); ?>" href="<?php echo U('Order/shouhou');?>" style="margin-top: 20px;">申请售后</a><?php endif; ?>
                </div>
